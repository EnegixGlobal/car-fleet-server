...
import { auth, uploadDutySlips } from '../middleware';
...
router.post('/:id/duty-slips', 
  auth(['admin', 'dispatcher']), 
  (req, res, next) => {
    uploadDutySlips(req, res, (err) => {
      if (err) {
...
router.get('/:id/driver-payments', auth(['admin','accountant','dispatcher','driver']), controller.listDriverPayments);
